...
const TarjetaDetalleDirectorio = (props) => {
    ...
    const [openDetalle, setOpenDetalle] = useState(open)
    const abrirDetalle = () => {
    ...
    return (
        <>
            <Row onClick={() => abrirDetalle()}>
                <Col style={{border:'1px solid #E7EAEC', padding: '1rem'}}>
            ...
            <Row>
            {openDetalle ?
                detalle.servicios!== undefined && detalle.servicios.map((l, i) => (
                <Col sm={12} lg={12} md={12} xs={12} style={{padding:'1rem'}}>

                    <label className='label-nombre-organizacion'>{l.NombreOrganizacion}</label>
                    <Col sm={12} lg={12} md={12} xs={12} className='d-flex'>
                        ...
                        <label className='label-telefono'>{l.telefono_.length > 0 ? l.telefono_[0].value : ''}</label>
                    ...
                    <Col sm={12} lg={12} md={12} xs={12}  className='d-flex'>
                        ...
                        <label className='label-horario'>{l.horario}</label>
                    </Col>
                </Col>
            ))
            : null}
